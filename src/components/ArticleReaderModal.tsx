...
export function ArticleReaderModal({ slug, lang, onClose, overrideArticle }: ArticleReaderModalProps) {
  ...
  const isVi = lang === 'vi';
  const d = article ? (article[lang] || article.vi || article.en) : null;

  useEffect(() => {
    if (article && d) {
      document.body.classList.add('reader-lock');
      document.title = `${d.title} — MC Xuân Hiến Blog`;
    } else {
      document.body.classList.remove('reader-lock');
      document.title = 'MC Xuân Hiến Blog';
    }
  ...
  }, [slug, overrideArticle, onClose, article, d]);

  if (!article || !d) return null;
        ...
        <div className="reader-panel overflow-y-auto p-6 sm:p-10 space-y-6">
          ...
          {d.coverImage && (
            <figure className="my-6 rounded-2xl overflow-hidden border border-slate-200 shadow-md">
              <img loading="lazy" decoding="async" src={d.coverImage} alt={d.title} className="w-full h-auto max-h-[520px] object-cover" />
            </figure>
          )}
              ...
              if (block.type === 'image') {
                return (
                  <figure key={bIdx} className="my-6 rounded-2xl overflow-hidden border border-slate-200 shadow-md bg-slate-950 text-white">
                    <img loading="lazy" decoding="async" src={block.content} alt={block.caption || 'Ảnh minh họa'} className="w-full h-auto max-h-[520px] object-cover" />
                    {block.caption && (
              ...
              if (block.t === 'img') {
                return (
                  <figure key={idx} className="my-6 rounded-2xl overflow-hidden border border-slate-200 shadow-md bg-slate-950 text-white">
                    <img loading="lazy" decoding="async" src={block.url} alt={block.caption || 'Hình ảnh minh họa'} className="w-full h-auto max-h-[520px] object-cover" />
                    {block.caption && (
                      <figcaption className="p-3 bg-slate-900/90 text-center font-mono text-xs text-slate-300 border-t border-slate-800">
                        📷 {block.caption}
                      </figcaption>
