  ...
  return (
    <div className="flex flex-col md:flex-row h-full gap-4 relative">
      ...
      <div className="w-full md:w-64 bg-white rounded-2xl border border-slate-200 p-3 shrink-0 flex flex-col gap-2 shadow-xs">
        ...
        <button 
          onClick={() => { setActiveTool('generate'); setGeneratedImage(null); }}
        ...
        <button 
          onClick={() => { setActiveTool('edit'); setGeneratedImage(null); }}
          className={`px-3 py-2.5 rounded-xl text-left text-xs font-bold flex items-center gap-2 transition-all ${
        ...
        <div className="mt-4 pt-4 border-t border-slate-100">
          ...
          <button disabled className="w-full px-3 py-2.5 rounded-xl text-left text-xs font-semibold flex items-center gap-2 text-slate-400 opacity-70 bg-slate-50">
            <UserCircle className="w-4 h-4 shrink-0" />
            Đổi Khuôn Mặt (FaceSwap)
          </button>
          
          <button disabled className="w-full mt-2 px-3 py-2.5 rounded-xl text-left text-xs font-semibold flex items-center gap-2 text-slate-400 opacity-70 bg-slate-50">
        ...
        {activeTool === 'edit' && (
          <div className="p-5 border-b border-slate-100 bg-slate-50">
            ...
            <div className="flex gap-4 mb-4">
              ...
              >
                {editSourceImage ? (
                  <img src={editSourceImage} alt="Source" className="w-full h-full object-cover" />
                ) : (
                  <>
        ...
        <div className="flex-1 p-5 flex flex-col items-center justify-center bg-slate-900 relative">
          {generatedImage ? (
            <div className="h-full w-full flex flex-col items-center justify-center animate-fade-in">
              <img src={generatedImage} alt="AI Result" className="max-h-[60vh] max-w-full rounded-lg shadow-2xl object-contain bg-slate-800" />
