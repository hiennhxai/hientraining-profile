              ...
              <div className="w-full sm:w-48 shrink-0 relative group">
                ...
                >
                  <div className="aspect-square rounded-2xl overflow-hidden border-2 border-orange-200 shadow-md bg-slate-100 relative">
                    <img src={gen.heroPortraitUrl || ""} 
                      alt="MC Nguyễn Hồng Xuân Hiến" 
                      loading="lazy" decoding="async" className="w-full h-full object-cover object-top group-hover:scale-105 transition-transform duration-500"
                    />
              ...
              >
                {trainingImages.map((imgUrl, idx) => (
                  <img key={idx}
                    src={imgUrl}
                    alt={`Training ${idx}`}
                    loading="lazy" decoding="async" className={`absolute inset-0 w-full h-full object-contain transition-opacity duration-500 ${idx === currentSlideIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}
                  />
                ))}
      ...
      {lightboxIndex !== null && createPortal(
        ...
        >
          ...
          <div className="absolute top-4 sm:top-6 left-4 sm:left-6 right-4 sm:right-6 flex justify-between items-center z-50 pointer-events-none">
            ...
            <button aria-label="Action button" onClick={(e) => { e.stopPropagation(); setLightboxIndex(null); }}
          ...
          {/* Nav Buttons */}
          <button aria-label="Action button" onClick={(e) => { e.stopPropagation(); prevSlide(); }}
          ...
          <button aria-label="Action button" onClick={(e) => { e.stopPropagation(); nextSlide(); }}
          ...
          >
            <img src={trainingImages[lightboxIndex]}
              alt="Training Gallery"
              loading="lazy" decoding="async" className="max-w-full max-h-[75vh] sm:max-h-[85vh] object-contain rounded-2xl shadow-2xl ring-1 ring-white/10 select-none"
            />
          </div>
        </div>,
        document.body
      ...
      {videoLightboxIndex !== null && createPortal(
        ...
        >
          ...
          <div className="absolute top-4 sm:top-6 left-4 sm:left-6 right-4 sm:right-6 flex justify-between items-center z-50 pointer-events-none">
            ...
            <button aria-label="Action button" onClick={(e) => { e.stopPropagation(); setVideoLightboxIndex(null); }}
              className="p-2.5 bg-white/10 hover:bg-white/25 text-white rounded-full backdrop-blur-md border border-white/20 transition-all shadow-lg pointer-events-auto hover:scale-105 cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
          ...
          {/* Nav Buttons */}
          <button aria-label="Action button" onClick={(e) => { e.stopPropagation(); setVideoLightboxIndex((prev) => prev !== null ? (prev === 0 ? trainingVideos.length - 1 : prev - 1) : null); }}
            className="absolute left-2 sm:left-8 p-2 sm:p-3 bg-white/10 hover:bg-white/25 text-white rounded-full backdrop-blur-md border border-white/20 transition-all z-50 hover:scale-110 shadow-lg cursor-pointer pointer-events-auto"
          >
            <ChevronLeft className="w-6 h-6 sm:w-8 sm:h-8" />
          </button>
          <button aria-label="Action button" onClick={(e) => { e.stopPropagation(); setVideoLightboxIndex((prev) => prev !== null ? (prev + 1) % trainingVideos.length : null); }}
            className="absolute right-2 sm:right-8 p-2 sm:p-3 bg-white/10 hover:bg-white/25 text-white rounded-full backdrop-blur-md border border-white/20 transition-all z-50 hover:scale-110 shadow-lg cursor-pointer pointer-events-auto"
          >
            <ChevronRight className="w-6 h-6 sm:w-8 sm:h-8" />
          </button>
