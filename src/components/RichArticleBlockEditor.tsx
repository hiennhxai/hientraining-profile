                    ...
                    <button aria-label="Action button" type="button"
                      onClick={() => {
                        if (onOpenPicker) {
                          ...
                          onOpenPicker(
                            (url) => {
                              const newC = `<img loading="lazy" decoding="async" src="${url}" alt="Article Image" class="w-full rounded-2xl shadow-md my-4" />`;
        ...
        <div className="fixed inset-0 z-[130] bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-3xl w-full p-6 space-y-4 border border-slate-200 shadow-2xl my-auto">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              ...
              <button aria-label="Action button" onClick={() => setShowPhotoPicker(null)} className="text-slate-400 hover:text-slate-600 text-xs font-bold">
                Đóng
              </button>
            </div>
