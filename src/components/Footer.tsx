...
export function Footer({ lang, onNavigatePage, onOpenAdminLogin, isEditActive = false, onEditField }: FooterProps) {
  ...
  const handleNav = () => {
    if (onNavigatePage) {
      // We no longer strictly need onNavigatePage for routing, but we keep it for any side effects
      onNavigatePage('');
    }
  };
    ...
    <footer className="border-t border-slate-200 bg-slate-50/80 py-7 text-xs text-slate-600 relative z-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-7">
          ...
          <div className="space-y-2.5 md:pl-4">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 font-mono text-xs font-semibold text-slate-700">
              <div className="space-y-1.5">
                <Link href="/" onClick={handleNav} className="block hover:text-orange-600 transition-colors cursor-pointer text-left">
                  › {isVi ? 'Trang chủ' : 'Home'}
                </Link>
                <Link href="/about" onClick={handleNav} className="block hover:text-orange-600 transition-colors cursor-pointer text-left">
                  › {t.nav_about}
                </Link>
                <Link href="/courses" onClick={handleNav} className="block hover:text-orange-600 transition-colors cursor-pointer text-left">
                  › {t.nav_courses}
                </Link>
                <Link href="/services" onClick={handleNav} className="block hover:text-orange-600 transition-colors cursor-pointer text-left">
                  › {t.nav_services}
                </Link>
              </div>

              <div className="space-y-1.5">
                <Link href="/projects" onClick={handleNav} className="block hover:text-orange-600 transition-colors cursor-pointer text-left">
                  › {t.nav_projects}
                </Link>
                <Link href="/blog" onClick={handleNav} className="block hover:text-orange-600 transition-colors cursor-pointer text-left">
                  › {t.nav_blog}
                </Link>
                <Link href="/contact" onClick={handleNav} className="block hover:text-orange-600 transition-colors cursor-pointer text-left">
                  › {t.nav_contact}
                </Link>
              </div>
            </div>
        ...
        <div className="pt-3.5 border-t border-slate-200/80 text-[11px] font-mono text-slate-500 flex items-center justify-between gap-3 flex-wrap">
          ...
          {onOpenAdminLogin && (
            <button aria-label="Action button" onClick={onOpenAdminLogin}
