...
export async function POST(request: Request) {
  try {
    ...
    const { name, phone, email, service, note, recaptchaToken } = body;
    ...
    if (smtpEmail && smtpPassword) {
      ...
      if (email) {
        await transporter.sendMail({
          from: `"MC Xuân Hiến" <${smtpEmail}>`,
          to: email,
          subject: 'Xác nhận Đăng ký Tư vấn / Khóa học - MC Xuân Hiến',
          html: `
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #333;">
              ...
              <p>MC Xuân Hiến hoặc trợ lý sẽ liên hệ với bạn trong thời gian sớm nhất qua số điện thoại bạn đã cung cấp để trao đổi chi tiết.</p>
              <p>Trân trọng,<br/><strong>Team MC Xuân Hiến</strong></p>
            </div>
          `,
        });
      }
